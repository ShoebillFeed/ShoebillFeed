...
| `TTS_SERVICE_TIMEOUT` | `120` (seconds) | Floor for the per-request timeout when `TTS_PROVIDER=network` — the actual timeout scales up with input text length beyond this, so a long script turn on a slow engine (e.g. Chatterbox, ~10x real-time on CPU) doesn't hit a flat cap. Raise this if even short requests are timing out against a slow/remote host. |
| `PODCAST_WORDS_PER_MINUTE` | `210` | Spoken words-per-minute used to size podcast scripts (both the LLM's word-count target and how many stories get selected for a given episode length). Calibrated against Piper's `en_US-libritts_r-medium` voice — if `TTS_ENGINE` (on `tts_service/`) is `kokoro` or `chatterbox` instead, tune this to that engine's actual pace or episodes will come out noticeably shorter/longer than the configured target length. For `chatterbox`, `195` is a reasonable starting point — real measured pace on its default English voice came out close to Piper's (~222 wpm actual across two synthesis samples) despite being far slower to *compute* (~10x real-time on CPU); this hasn't been checked against cloned voices or other languages, so verify against a real generated episode. |
