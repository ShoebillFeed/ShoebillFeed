...
export default function PodcastsPanel() {
  ...
  return (
    <Accordion
      ...
      action={
        <button
          onClick={() => { setEditingShowId(null); setShowForm(true); }}
          ...
          <div key={show.id} className="flex flex-col gap-2">
            <div className="flex items-center gap-3 p-4 bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-lg">
              ...
              <div className="flex items-center gap-1">
                ...
                <IconButton
                  title={t("common.edit")}
                  onClick={() => setEditingShowId(editingShowId === show.id ? null : show.id)}
                  className={editingShowId === show.id ? "text-indigo-600 dark:text-indigo-400" : ""}
                >
                  <Pencil size={14} />
                </IconButton>
                <IconButton
            ...
            {editingShowId === show.id && (
              <div className="p-4 border border-indigo-200 dark:border-indigo-800 rounded-lg bg-gray-50 dark:bg-gray-800/50">
                <PodcastShowForm show={show} onClose={() => setEditingShowId(null)} />
              </div>
            )}
          </div>
        ))}
